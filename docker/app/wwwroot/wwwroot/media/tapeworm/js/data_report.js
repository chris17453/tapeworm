(function ( $ ) {
    ...
    $.fn.datareport = function( options ) {
		var settings = $.extend({
		...
		//********************************************************************************************
		var datareport=function(element,options){
			...
			this.id= 'datareport_' + Math.random().toString(36).substr(2, 9);
    ...
    datareport.prototype.build_report_filters=function(){
            ...
            for (var property_ordinal in property_ordinals) {
                ...
                if ( property.filter && property.options.length>0) {      //if filter and has select options
                ...
                if(property.options && property.options.Length>0){
                    for(var oindex in property.options){
                        ...
                        if(option.default){
                            ...
                            filterDefaults[index]= option.value;
                        }
                    }
                }
        ...
        datareport.prototype.build=async function(){
			...
			<div class="input-group mb-3">
			  <div class="input-group-prepend">
				<span class="input-group-text" id="basic-addon1"><span data-feather="search"></span></span>
			  </div>
			  ...
			  <div class="input-group-append">
				<span class="input-group-text ${this.multi_search_reset_class}" data-uid="${this.id}"><span data-feather="trash"></span></span>
			  </div>
			...
			var pagination=`<nav aria-label="pagination pagination-sm">
			  <ul class="pagination ts-pager">
				...
				<li class=" page-item"><span class="page-link ${this.id}_page_display pager-nowrap" data-pager-output-filtered="{startRow:input} – {endRow} / {filteredRows} of {totalRows} total rows"></span></li>
        ...
        datareport.prototype.bind=function (){
            var self=this;
		...
		datareport.prototype.unbind=function(){
            //clear variables
			//$(this.element).on("click",`.${this.multi_search_class}`      );
			$(this.element).on("click",`.${this.multi_search_reset_class}`);
		}
        datareport.prototype.destroy=function(){
            this.unbind();
            $(this.element).html('');       //cleanup injected HTML.
		}
        datareport.prototype.close=function(){
            this.destroy();
		}
        //********************************************************************************************
		// data-report ajax function
		//*******************************************************************************************
        datareport.prototype.call_api =async function(method,da){
...
/*					headers    : {  'Access-Control-Allow-Credentials' : true,
		...
		//********************************************************************************************
        datareport.prototype.reset=function(e){
		...
		//********************************************************************************************
        datareport.prototype.multi_search=function(report_uid){
			$(this.table).trigger('pagerUpdate');                           
			$(this.table).trigger('Update');                                
        }
        //********************************************************************************************
		...
		//********************************************************************************************
        datareport.prototype.load_data_config=async function(report_uid){
			var results=await this.call_api(this.options.config_method,{uid:report_uid});
			this.data_config=results;
			var x=this;
        }
        datareport.prototype.build_api_url=function(){
        ...
        // data-report gab configuration data
        //********************************************************************************************
        datareport.prototype.ajax_error=function(){
            return false;
		}
		//********************************************************************************************
		// Tablesorter ajax object actualy sent to webAPI
		//********************************************************************************************
        datareport.prototype.build_data_object=function(data_uid,page,page_length,sort_options=null,filter_options=null,multi_search=null){
			var data_object={
		...
		//********************************************************************************************
		datareport.prototype.ajax_processing=function(data) {
				...
				for (record = 0; record < page_length; record++) {
					...
					if(record>=len) {
						...
						for(var i in config.property_ordinals) {
							...
							row[i] = "<span>" + d[record]['model'][name] + "</span>";
						}
					}
		...
		//*******************************************************************************************
		datareport.prototype.table_sorter_custom_ajax=function (table, url) {
		...
		//*******************************************************************************************
		datareport.prototype.init_tablesorter=function () {
			var widgets=[];
			var widget_options={};
	    ...
	    datareport.prototype.obj_to_string=function(obj){
            return  JSON.stringify(obj);
        }
        return this.each(function() {
            new datareport(this,settings);
        });
